...
- 作业模板设计器新增完整的 Slurm 资源申请组件库：
  - 支持作业名称、项目 Account、队列、节点数、任务数、每任务 CPU、GPU、内存、运行时长、工作目录、标准输出、错误输出、邮件通知、QOS、节点约束、作业数组、独占节点和高级 Slurm 参数。
  - 支持输入框、数字输入、下拉框、单选、多选、开关、滑动条、文件、目录和自定义变量组件。
  - 下拉类组件支持固定选项和动态数据源两种模式，固定选项可维护显示名称与实际值。
  - 动态项目 Account、队列和 QOS 按当前用户授权实时加载，外部查询失败时提供中文降级提示。
- 作业提交新增资源约束联动：
  - 根据所选队列展示 CPU、GPU、GRES、最长运行时间和节点信息。
  - 根据队列资源上限动态约束 CPU/GPU 输入范围，减少无效资源申请。
  - 后端按照模板字段生成对应 `#SBATCH` 参数，并兼容旧版模板字段。
- 项目中心新增作业资源用量统计：
  - 项目作业自动按 Slurm Account 归集，同时兼容人工关联记录。
  - 展示项目作业数、CPU 核时、GPU 卡时、异常作业和完成进度。
  - Slurm 作业同步新增运行秒数、CPU 时间和 `AllocTRES` 数据。
- 新增数据库迁移 `017_project_job_usage`，补充项目作业用量统计字段和 Account 用量查询索引。
- 新增项目作业访问控制：项目成员可按项目权限查看作业，项目管理者可执行项目范围内的管理操作。

### Changed

- 作业模板编辑器改为固定头部和底部、三列独立滚动的工作台布局，组件库、设计画布和组件设置可分别滚动。
- 作业模板提交页面默认提供常用 Slurm 资源字段，项目、队列和资源限制随用户权限动态变化。
- 项目 Slurm Account 同步会回收已移除成员的过期关联；当关联仍是用户默认 Account 时停止危险操作并返回明确提示。
- 项目中心视觉调整为高密度运维工作台，使用统一线性图标、实体操作按钮和紧凑信息分区。
- 全站 34 个页面统一系统字体、标题格式、按钮、输入框、卡片、弹窗、圆角、遮罩和阴影规范。
- 页面主题资源增加统一缓存版本，避免部署后浏览器继续使用旧样式。

### Fixed

- 修复作业模板编辑弹窗底部裁剪、侧栏无法滚动和保存按钮被内容遮挡的问题。
- 修复切换模板组件后右侧组件设置仍显示上一个组件数据的问题。
- 修复下拉框组件无法维护固定选项、动态数据源缺失的问题。
- 修复应用广场、模板管理、授权审批刷新后总是返回应用广场的问题。
- 修复 License 配置子页面刷新后返回默认页的问题，当前子页面写入 URL 并支持浏览器前进/后退。
- 修复项目作业统计缺少运行时长、CPU 时间和 GPU 用量来源的问题。
- 修复删除项目成员后 Slurm Account 关联未同步回收的问题。
- 修复部分独立页面未加载共享主题、字体和控件风格不一致的问题。

### Deployment

- 测试服务器 `/data/simpleHPC` 已部署并验证：
  - `simplehpc-backend.service` 保持 active。
  - 34 个受管页面及共享主题资源均返回 200。
  - 项目中心、License 配置、权限管理和巡检日志完成桌面端浏览器检查。
  - 共享表单、菜单总览、License 详情和作业模板编辑器完成弹窗交互检查。
  - `480 x 844` 移动端视口无横向溢出，作业模板三栏和底部操作区可正常滚动与操作。
  - 浏览器控制台未发现页面级 error 或 warn。

### Security

- 动态 Slurm 参数名和值均经过后端白名单和安全字符校验，拒绝换行、控制字符及危险参数。
- 项目 Account 在提交作业前同时校验项目成员关系和 Slurm Association，未授权 Account 不会写入脚本。
- 项目作业详情和操作接口增加项目成员/管理权限校验，不依赖前端传入的权限字段。
- Slurm 用户 Account 关联回收使用参数化 `sacctmgr` 调用，不拼接 shell 命令。
- 本版本不包含真实 `.env`、密码、SSH 私钥、数据库 dump、运行日志和后端构建二进制。
